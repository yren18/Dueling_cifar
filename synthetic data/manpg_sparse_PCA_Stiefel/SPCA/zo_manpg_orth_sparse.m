function [X_manpg, F_manpg,sparsity,time_manpg,iter,flag_succ,num_linesearch,mean_ssn] = zo_manpg_orth_sparse(B,option)
...
% Zeroth order oracle:
nu = 1e-7; % the smoothing parameter
m = n; % the number of multi-sampling
if type == 1
    AX = zero_oracle(X, B, nu, m);
else
    AX = zero_oracle(X, B.'*B, nu, m);
end

if type == 1
    F(1) = -sum(sum(X.*(B*X)))+h(X);
else
    F(1) = -sum(sum(X.*(B.'*(B*X))))+h(X);
end
...
%     end
    ...
    if type == 1
        AZ = zero_oracle(Z, B, nu, m);
    else
        AZ = zero_oracle(Z, B.'*B, nu, m);
    end
    %   AZ = real(ifft( LAM_manpg.*fft(Z) ));
    
    if type == 1
        f_trial = -sum(sum(Z.*(B*Z)));
    else
        f_trial = -sum(sum(Z.*(B.'*(B*Z))));
    end
